...
def generate_response_with_groq(query, context_docs, chat_history=[]):
    ...
    for turn in chat_history[-5:]:  # Use last 5 turns
        history_context += f"User: {turn['user_input']}\nAssistant: {turn['response'][0]['section_summary']}\n"
...
def home():
    ...
    if request.method == 'POST':
        if request.form.get('clear'):
            session['chat_history'] = []
            return render_template('index1.html', result=[], user_input="", chat_history=[])
        ...
        if not user_input.strip():
            ...
            return render_template('index1.html', result=result, user_input=user_input, chat_history=session['chat_history'])
